...
export default function EditReceiverPage() {
    ...
    const { canManuallyPassed, canPdf, canReScreening, canDilisenseScreening, canDeleteComplianceReport } = usePagePermissions('RECEIVERS');
                        ...
                        <div className="mb-6 flex items-start justify-between gap-4 border-b border-slate-100 dark:border-slate-800 pb-4">
                            <div>
                                ...
                                <p className="mt-1 text-sm font-medium text-slate-500 dark:text-slate-400">
                                    Manage, view, run checks, or delete AML Reports for {formData.name || '-'}.
                                </p>
                            </div>
                            <button
                                type="button"
                    ...
                    <div className="relative z-10 w-full max-w-md rounded-3xl border border-slate-200/50 bg-white p-6 shadow-2xl dark:border-slate-700/50 dark:bg-slate-900 transform scale-100 transition-all duration-300">
                        <div className="mb-4 text-center">
                            ...
                            <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
                                Are you sure you want to rescreen the receiver?
                            </p>
                        </div>
                                ...
                                onClick={async () => {
                                    ...
                                    try {
                                        const res = await fetch(
                                            withActingUserParam(
                                                ENDPOINTS.BENEFICIARIES.DILISENSE_REPORT_GENERATE(id),
                                                currentUser
                                            ),
                                            {
                                                method: 'POST',
                                        ...
                                        if (res.ok) {
                                            const listRes = await fetch(
                                                withActingUserParam(
                                                    ENDPOINTS.BENEFICIARIES.DILISENSE_REPORTS_LIST(id),
                                                    currentUser
                                                )
                                            );
                                            const listData = await listRes.json().catch(() => []);
                                            ...
                                            if (receiverRes.ok) {
                                                ...
                                                setFormData((prev: any) => ({
                                                    ...prev,
                                                    aml_status: rData.aml_status ?? 'passed',
                                                    aml_reference: rData.aml_reference ?? '',
                                                    aml_checked_at: rData.aml_checked_at ?? '',
                                                    aml_raw_payload: rData.aml_raw_payload ?? '',
                                                    aml_screening_doc: rData.aml_screening_doc ?? '',
                                                    aml_score: Number(rData.aml_score ?? 0),
                                                }));
                                            }
                                            ...
                                            if (newReportId) {
                                                void openPdfReport(
                                                    withActingUserParam(
                                                        ENDPOINTS.BENEFICIARIES.DILISENSE_REPORT_DOWNLOAD(id, newReportId),
                                                        currentUser
                                                    ),
                                                    currentUser,
                                                    pdfWindow
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        {canPdf && (
                            <button
                                ...
                                className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full text-xs font-extrabold bg-teal-500 hover:bg-teal-600 text-white shadow-md shadow-teal-500/20 transition-all cursor-pointer"
                            >
