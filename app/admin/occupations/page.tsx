...
const EMPTY_FORM: OccupationFormState = {
    name: '',
    active: 'yes',
};
...
export default function OccupationsPage() {
    const { showCreatedBy, showCreatedAt, showUpdatedBy, showUpdatedAt } = useAuditColumns('OCCUPATIONS');
    const { canAdd, canEdit, canDelete } = usePagePermissions('OCCUPATIONS');
    ...
    const [form, setForm] = useState<OccupationFormState>(EMPTY_FORM);
    ...
    const fetchOccupations = async () => {
        setLoading(true);
        try {
            const res = await fetch(`${ENDPOINTS.OCCUPATIONS.LIST}?_t=${Date.now()}`, {
                cache: 'no-store',
            });
            if (res.ok) {
    ...
    const openEditModal = (row: OccupationRow) => {
        setEditingId(row.id);
        setForm({
    ...
    const handleSubmit = async (event: React.FormEvent) => {
        ...
        try {
            ...
            const res = await fetch(
                editingId ? ENDPOINTS.OCCUPATIONS.DETAIL(editingId) : ENDPOINTS.OCCUPATIONS.LIST,
                {
            ...
            if (res.ok) {
                ...
                setConfirmModal({
                    isOpen: true,
                    title: 'Saved',
                    message: editingId ? 'Occupation updated.' : 'Occupation added.',
                    type: 'info',
                    isAlert: true,
                });
                return;
            ...
            const message = await readErrorMessage(res, 'Failed to save occupation.');
            setConfirmModal({
                isOpen: true,
                title: 'Error',
                message,
            ...
            setConfirmModal({
                isOpen: true,
                title: 'Error',
                message: 'An error occurred while saving the occupation.',
                type: 'danger',
                isAlert: true,
            });
        } finally {
    ...
    const handleDelete = async () => {
        ...
        if (!canDelete) {
            setConfirmModal({
                ...
                message: 'You do not have permission to delete occupations.',
                type: 'warning',
                isAlert: true,
            });
            setDeleteId(null);
        ...
        try {
            const res = await fetch(ENDPOINTS.OCCUPATIONS.DETAIL(deleteId), { method: 'DELETE' });
            if (res.ok) {
                await fetchOccupations();
                setConfirmModal({
                    isOpen: true,
                ...
                const message = await readErrorMessage(res, 'Failed to delete occupation.');
                setConfirmModal({
                    isOpen: true,
                    title: 'Error',
                    message,
            ...
            setConfirmModal({
                isOpen: true,
                title: 'Error',
                message: 'An error occurred while deleting the occupation.',
                type: 'danger',
                isAlert: true,
            });
        } finally {
    ...
    return (
        <div className="max-w-7xl mx-auto space-y-8 animate-fade-in-up">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight">Occupations</h1>
                    <p className="text-slate-500 dark:text-slate-400 mt-2 font-medium">
                        Maintain occupation options used in remitter profile creation.
                    </p>
                </div>
            ...
            <div className="card-glass overflow-hidden shadow-xl">
                <div className="px-6 py-4 border-b border-slate-100/70 dark:border-slate-700/60 flex items-center space-x-3">
                    ...
                    <div>
                        <h2 className="text-xl font-bold text-slate-900 dark:text-white">Occupations Directory</h2>
                    ...
                    <table className="table-shell">
                        ...
                        <tbody className="table-body">
                            ...
                            {!loading && pagedRows.length === 0 && (
                                <tr><td colSpan={3 + (canEdit ? 1 : 0) + (canDelete ? 1 : 0) + (showCreatedBy ? 1 : 0) + (showCreatedAt ? 1 : 0) + (showUpdatedBy ? 1 : 0) + (showUpdatedAt ? 1 : 0)} className="px-6 py-10 text-center text-slate-500">No occupations found.</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
            ...
            <Modal
                isOpen={modalOpen}
                title={editingId ? 'Edit Occupation' : 'Add Occupation'}
                ...
                <form className="space-y-5" onSubmit={handleSubmit}>
                    <div>
                        ...
                        <input
                            ...
                            placeholder="Doctor, Engineer, Accountant, etc"
                            required
                        />
                    </div>
                    <div>
            ...
            <ConfirmModal
                ...
                message="This will permanently remove the occupation. Continue?"
                type="danger"
                confirmText="Delete"
...
function getSortValue(row: OccupationRow, key: SortKey) {
    switch (key) {
        case 'name':
