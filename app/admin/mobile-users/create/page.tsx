    ...
    return (
        <div>
            ...
            <div 
                ...
                className={`border border-dashed border-slate-300 dark:border-slate-600 rounded-lg ${compact ? 'px-3 py-2' : 'px-4 py-6'} bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors cursor-pointer text-center relative max-w-full overflow-hidden`}
            >
                <div className="flex flex-col items-center justify-center">
                    {previewUrl ? (
                        <div className="relative w-16 h-16 mb-2 rounded border border-slate-200 overflow-hidden">
                            <img src={previewUrl} alt="Preview" className="w-full h-full object-cover" />
                        </div>
                    ) : selectedFile ? (
                        <div className="w-8 h-8 rounded bg-slate-100 dark:bg-slate-700 flex items-center justify-center mb-2">
                            <FileText className="w-4 h-4 text-teal-500 animate-pulse" />
                        </div>
                    ...
                    <span className="text-xs text-slate-500 dark:text-slate-400 truncate w-full px-2">
                        {selectedFile ? (
                            <span className="text-teal-600 dark:text-teal-400 font-medium flex items-center justify-center gap-1">
                                <CheckCircle className="w-3.5 h-3.5 shrink-0" /> {selectedFile.name}
                            </span>
                        ) : displayExisting ? (
                            <span className="text-slate-600 dark:text-slate-300 font-medium flex items-center justify-center gap-1">
