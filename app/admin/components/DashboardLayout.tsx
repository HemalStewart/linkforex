...
export default function DashboardLayout({ children }: DashboardLayoutProps) {
    ...
    return (
        <div className="admin-shell flex h-screen overflow-hidden antialiased text-slate-900 dark:text-white">
            <aside className={`admin-sidebar z-20 my-4 ml-4 flex flex-col rounded-[30px] transition-all duration-500 ease-in-out ${sidebarOpen ? 'w-[300px]' : 'w-[96px]'}`}>
                <div className="border-b border-white/10 px-4 py-4 dark:border-white/6">
                    <div className={`flex items-center ${sidebarOpen ? 'justify-between gap-3' : 'justify-center'} rounded-[22px] bg-white/16 p-2.5 dark:bg-white/4`}>
                        <button
                            onClick={() => router.push('/admin/dashboard')}
                            className={`flex min-w-0 items-center gap-3 ${sidebarOpen ? '' : 'justify-center'}`}
                ...
                <nav className="flex-1 overflow-y-auto px-3 py-5 no-scrollbar">
                    {navigation.map((item, idx) => {
                        ...
                        if (item.children) {
                            return (
                                <div key={item.name} className="mb-4 stagger-item" style={{ animationDelay: `${idx * 0.05}s` }}>
                                    {sidebarOpen && (
                                        <div className="mb-2 px-3 text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-400 dark:text-slate-500">
                                            {item.name}
                                        </div>
                                    )}
                                    <button
                                    ...
                                    >
                                        <div className="flex items-center space-x-3.5">
                                            ...
                                            {sidebarOpen && <span className="tracking-wide text-[15px] font-semibold">{item.name}</span>}
                                        </div>
                                        {sidebarOpen && (
                        ...
                        return (
                            <Link
                                key={item.name}
                                href={item.href!}
                                className={`admin-nav-button ${sidebarOpen ? 'justify-between px-3.5 py-3.5' : 'justify-center px-2 py-3.5'} mb-3 stagger-item ${isActive ? 'admin-nav-button-active font-semibold' : ''}`}
            ...
            <div className="relative flex min-w-0 flex-1 flex-col overflow-hidden">
                <header className="admin-topbar z-10 mx-4 mb-4 mt-4 flex min-h-[78px] items-center justify-between gap-4 rounded-[28px] px-5 py-4 animate-slide-down">
                    ...
                    <div className="flex items-center gap-3">
                        <Link
                            ...
                            className="btn-secondary hidden items-center space-x-2 rounded-full px-4 py-3 text-slate-700 dark:text-slate-200 md:inline-flex"
                        >
