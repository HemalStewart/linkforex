...
export default function RelationshipsPage() {
    ...
    const { canAdd, canEdit, canDelete } = usePagePermissions('RELATIONSHIPS');
    ...
    const handleSubmit = async (event: React.FormEvent) => {
        ...
        if (!form.name.trim()) {
            setConfirmModal({
                isOpen: true,
                title: 'Missing name',
                message: 'Please enter a relationship name.',
                type: 'warning',
                isAlert: true,
            });
            return;
        ...
        try {
            ...
            if (res.ok) {
                ...
                setConfirmModal({
                    isOpen: true,
                    title: 'Saved',
                    message: editingId ? 'Relationship updated.' : 'Relationship added.',
                    type: 'info',
                    isAlert: true,
                });
                return;
            ...
            const message = await readErrorMessage(res, 'Failed to save relationship.');
            setConfirmModal({
                isOpen: true,
                title: 'Error',
                message,
            ...
            setConfirmModal({
                isOpen: true,
                title: 'Error',
                message: 'An error occurred while saving the relationship.',
                type: 'danger',
                isAlert: true,
            });
        } finally {
    ...
    const handleDelete = async () => {
        ...
        if (!canDelete) {
            setConfirmModal({
                ...
                message: 'You do not have permission to delete relationships.',
                type: 'warning',
                isAlert: true,
            });
            setDeleteId(null);
        ...
        try {
            ...
            if (res.ok) {
                ...
                const message = await readErrorMessage(res, 'Failed to delete relationship.');
                setConfirmModal({
                    isOpen: true,
                    title: 'Error',
                    message,
        ...
        <div className="max-w-7xl mx-auto space-y-8 animate-fade-in-up">
            ...
            <div className="card-glass overflow-hidden shadow-xl">
                <div className="px-6 py-4 border-b border-slate-100/70 dark:border-slate-700/60 flex items-center space-x-3">
                    ...
                    <div>
                        <h2 className="text-xl font-bold text-slate-900 dark:text-white">Relationships Directory</h2>
                    ...
                    <table className="table-shell">
                        ...
                        <tbody className="table-body">
                            ...
                            {!loading && pagedRows.length === 0 && (
                                <tr><td colSpan={3 + (canEdit ? 1 : 0) + (canDelete ? 1 : 0) + (showCreatedBy ? 1 : 0) + (showCreatedAt ? 1 : 0) + (showUpdatedBy ? 1 : 0) + (showUpdatedAt ? 1 : 0)} className="px-6 py-10 text-center text-slate-500">No relationships found.</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
            ...
            <Modal
                isOpen={modalOpen}
                title={editingId ? 'Edit Relationship' : 'Add Relationship'}
            ...
            <ConfirmModal
                ...
                message="This will permanently remove the relationship. Continue?"
                type="danger"
                confirmText="Delete"
